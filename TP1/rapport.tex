...
\usepackage[french]{babel} % O curso é em francês, mas você pode ajustar se preferir
\usepackage{amsmath}
\usepackage{graphicx}
\usepackage{booktabs}
\usepackage{hyperref}
...
\author{Julia Ellen Dias Leite - Helena Guachalla de Andrade - Mateus Bastos Soares}
\date{\today}
...
L'expérience a été réalisée sur uma imagem de dimensions \textbf{240 lignes x 360 colonnes}. Les temps d'exécution mesurés pour l'application du noyau de convolution sont les suivants :
...
\begin{itemize}
    ...
    \item \textbf{Affichage (\texttt{plt.imshow}) :} Fonction de MatPlotLib utilisée para visualiser les résultats. Comme MatPlotLib attend du RGB, une conversion est souvent nécessaire pour obtenir les couleurs réelles.
    \item \textbf{Convolution (\texttt{cv2.filter2D}) :} Applique un noyau (kernel) linéaire à l'image de manière optimisée.
\end{itemize}

%Q2: Expliquer pourquoi le noyau de convolution fourni en exemple r´ealise un r´ehaussement de
%contraste par rapport `a l’image originale
